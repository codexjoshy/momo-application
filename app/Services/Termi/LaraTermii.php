...
class LaraTermii implements SmsServiceInterface
{
    /*
     *
     * Private Props
    ...
    {
        $this->key = config("termii.apiKey");
        $this->apiUrl = config("termii.baseUrl");
        $this->from = config("termii.from");
        $this->channel = config("termii.channel");
    }
	...
	*/
    ...
    {
        // $url = $this->base("/get-balance?api_key={$this->key}");
        // dd($url);
	...
	*/
    public function sendSms(string $to, string $message, ?string $from = null): string|array
    {
        ...
        $url = 'sms/send?' . $queryString;
        $request = Http::post($this->base("$url"));
        dd($request);
        $status = $request->status();
        //There is a fix here
        //TODO: Fix
	...
	*/
    public function sendBulkSms(array $to, string $message, ?string $from = null): string|array
    {
        ...
        $request = Http::post($this->base("sms/send/bulk"), $data);
        $status = $request->status();
        //There is a fix here
        //TODO: Fix
        if (json_decode($this->checkStatus($status)->content())->success || $status === 400) {
